...
The function `distributionDP()` estimates private histograms from the differentialy private data. There are three required arguments. `variable` should be the name of the column in the data, `data` is that data name, and `distsributions` is a vector of distributions to paramaterize. 


```{r}
set.seed(02138)

dist_test <- distributionDP(variable = X1, data = dp_data2, distributions = c('Normal', 'Poisson', 'ZIP', 'NB', 'ZINB'), moments_fit = 6, plot = TRUE, plot_dp = TRUE)
...
# Here we can check this is right by comparing the paramaterized distribution to the private data
...
```{r, echo = FALSE, warning = FALSE}
 ggplot2::ggplot(private_data2) +
      ...
      ggplot2::scale_fill_manual(values = c('navyblue')) + xlim(c(-5, 20))
```
...
# Validate they are ormally distributed 
