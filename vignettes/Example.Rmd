...
system.time(bootstrap <- lmdp(Y ~ X1 + X2 + X3, data = dp_data, bootstrap_var = TRUE, nsims_var = 10))
...
lmdp_interaction <- lmdp(Y ~ X1 + X2 + X3 + X1*X2, data = dp_data, nsims_var = 10)
...
```
\
Other variable transformations, or multiple variable transformation, are not allowed in this version of the package and their inclusion will induce an error message. Note also that `lmdp()` currently only supports bootstrap estimation when the model includes transformed variables. We are working on expanding the set of admisssible variable transformations and introducing the simulation approach to variance estimation for these models. 
